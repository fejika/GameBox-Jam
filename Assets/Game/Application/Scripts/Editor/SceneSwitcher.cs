...
public static class SceneSelectionToolbar
{
    ...
    static void AddScene(SceneInfo scene)
    {
        ...
        if (_scenes.Any(s => s.Path == scene.Path) || _scenes.Any(s => s.Name == scene.Name))
            RemoveScene(scene);
    ...
    private static void RemoveCorrectScene() {
        var sceneInfo = _scenes[_selectedIndex - 1].Name == _sceneOpened.Name
            ? _sceneOpened
            : _scenes[_selectedIndex - 1];
        RemoveScene(sceneInfo);
    }
    
    static void RemoveScene(SceneInfo scene) {
        var sceneInfo = _scenes.First(info => info.Name == scene.Name);
